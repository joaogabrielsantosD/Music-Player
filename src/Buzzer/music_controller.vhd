...
architecture arc of music_controller is
   ...
   -- Formula: Overflow = (50MHz / (2*f)) - 1
	constant PAUSA : integer := 0; -- buzzer stopped
 
	constant C3 : integer := 95555; -- 261.63 Hz (Dó3)
	constant D3 : integer := 85130; -- 293.66 Hz (Ré3)
	constant E3 : integer := 75842; -- 329.63 Hz (Mi3)
	constant F3 : integer := 71585; -- 349.23 Hz (Fá3)
	constant G3 : integer := 63775; -- 392.00 Hz (Sol3)
	constant A3 : integer := 56817; -- 440.00 Hz (Lá3)
	constant B3 : integer := 50618; -- 493.88 Hz (Si3)
	constant Cs3 : integer := 90192; -- 277.18 Hz (Dó#3/Réb3)
	constant Ds3 : integer := 80349; -- 311.13 Hz (Ré#3/Mib3)
	constant Fs3 : integer := 67567; -- 369.99 Hz (Fá#3/Solb3)
	constant Gs3 : integer := 60166; -- 415.30 Hz (Sol#3/Láb3)
	constant As3 : integer := 53628; -- 466.16 Hz (Lá#3/Sib3)

	constant C4 : integer := 47777; -- 523.25 Hz (Dó4)
	constant D4 : integer := 42565; -- 587.33 Hz (Ré4)
	constant E4 : integer := 37921; -- 659.25 Hz (Mi4)
	constant F4 : integer := 35792; -- 698.46 Hz (Fá4)
	constant G4 : integer := 31887; -- 783.99 Hz (Sol4)
	constant A4 : integer := 28408; -- 880.00 Hz (Lá4)
	constant B4 : integer := 25308; -- 987.77 Hz (Si4)
	constant Cs4 : integer := 45096; -- 554.37 Hz (Dó#4/Réb4)
	constant Ds4 : integer := 40174; -- 622.25 Hz (Ré#4/Mib4)
	constant Fs4 : integer := 33783; -- 739.99 Hz (Fá#4/Solb4)
	constant Gs4 : integer := 30083; -- 830.61 Hz (Sol#4/Láb4)
	constant As4 : integer := 26813; -- 932.33 Hz (Lá#4/Sib4)

	constant C5 : integer := 23888; -- 1046.50 Hz (Dó5)
	constant D5 : integer := 21282; -- 1174.66 Hz (Ré5)
	constant E5 : integer := 18960; -- 1318.51 Hz (Mi5)
	constant F5 : integer := 17895; -- 1396.91 Hz (Fá5)
	constant G5 : integer := 15943; -- 1567.98 Hz (Sol5)
	constant A5 : integer := 14203; -- 1760.00 Hz (Lá5)
	constant B5 : integer := 12653; -- 1975.53 Hz (Si5)
	constant Cs5 : integer := 22547; -- 1108.73 Hz (Dó#5/Réb5)
	constant Ds5 : integer := 20087; -- 1244.51 Hz (Ré#5/Mib5)
	constant Fs5 : integer := 16891; -- 1479.98 Hz (Fá#5/Solb5)
	constant Gs5 : integer := 15041; -- 1661.22 Hz (Sol#5/Láb5)
	constant As5 : integer := 13406; -- 1864.66 Hz (Lá#5/Sib5)
	...
	type vetor_pokemon is array (0 TO 61) of score; -- Pokemon - Cerulean Theme (62)
	type vetor_myway is array (0 TO 44) of score; -- Frank Sinatra - My Way     (45)
		
	--Take On Me
	constant music1 : vetor_takeonme := (
		(note => PAUSA, time_t => ov2_t1_2),
		(note => Fs4, time_t => ov2_t1_2), (note => Fs4, time_t => ov2_t1_2), (note => D4,  time_t => ov2_t1_2), 
		(note => B3,  time_t => ov2_t1_2), (note => PAUSA, time_t => ov2_t1_2), (note => B3,  time_t => ov2_t1_2), 
		(note => PAUSA, time_t => ov2_t1_2), (note => E4,  time_t => ov2_t1_2), (note => PAUSA, time_t => ov2_t1_2),
		(note => E4,  time_t => ov2_t1_2), (note => PAUSA, time_t => ov2_t1_2), (note => E4,  time_t => ov2_t1_2),
		...
		(note => A4,  time_t => ov2_t1_2), (note => E4,  time_t => ov2_t1_2), (note => PAUSA, time_t => ov2_t1_2),
		(note => D4,  time_t => ov2_t1_2), 
		(note => PAUSA, time_t => ov2_t1_2),
		(note => Fs4, time_t => ov2_t1_2), (note => PAUSA, time_t => ov2_t1_2), (note => Fs4, time_t => ov2_t1_2),
		(note => PAUSA, time_t => ov2_t1_2), (note => Fs4, time_t => ov2_t1_2), (note => E4,  time_t => ov2_t1_2),
	...
	constant music2 : vetor_pokemon := (
		...
		(note => PAUSA, time_t => ov3_t1_4),
		(note => E4,  time_t => ov3_t1_2), (note => B3,  time_t => ov3_t1_2), (note => Cs4, time_t => ov3_t1_2),
		(note => Ds4, time_t => ov3_t1_2), (note => E4,  time_t => ov3_t1_2), (note => Fs4, time_t => ov3_t1_2),
		(note => Gs4, time_t => ov3_t1_2), (note => A4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2),
		(note => A4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2), (note => Fs4, time_t => ov3_t1),
		(note => PAUSA, time_t => ov3_t1_4),
		(note => E4,  time_t => ov3_t1_2), (note => B3,  time_t => ov3_t1_2), (note => Cs4, time_t => ov3_t1_2),
		(note => Ds4, time_t => ov3_t1_2), (note => E4,  time_t => ov3_t1_2), (note => Fs4, time_t => ov3_t1_2),
		(note => Gs4, time_t => ov3_t1_2), (note => A4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2),
		(note => E4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2), (note => B4,  time_t => ov3_t1),
		(note => PAUSA, time_t => ov3_t1_4),
		(note => E4,  time_t => ov3_t1_2), (note => B3,  time_t => ov3_t1_2), (note => Cs4, time_t => ov3_t1_2),
		(note => Ds4, time_t => ov3_t1_2), (note => E4,  time_t => ov3_t1_2), (note => Fs4, time_t => ov3_t1_2),
		(note => Gs4, time_t => ov3_t1_2), (note => A4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2),
		(note => A4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2), (note => Fs4, time_t => ov3_t1),
		(note => PAUSA, time_t => ov3_t1_4),
		(note => Gs3, time_t => ov3_t1), (note => E4,  time_t => ov3_t1_2), (note => B3,  time_t => ov3_t1_2),
		(note => Cs4, time_t => ov3_t1_2), (note => Ds4, time_t => ov3_t1_2), (note => E4,  time_t => ov3_t1_2),
		(note => Fs4, time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2), (note => A4,  time_t => ov3_t1_2),
		(note => Gs4, time_t => ov3_t1_2), (note => E4,  time_t => ov3_t1_2), (note => Gs4, time_t => ov3_t1_2),
		(note => B4,  time_t => ov3_t2)
	);

	-- On my way
	constant music3 : vetor_myway := (
		(note => Fs4, time_t => ov4_t1), (note => G4,  time_t => ov4_t1), (note => A4,  time_t => ov4_t1), 
		(note => A4,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), (note => A4,  time_t => ov4_t1), 
		(note => Gs4, time_t => ov4_t1), (note => A4,  time_t => ov4_t1), (note => A4,  time_t => ov4_t1),
		(note => A4,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1),
		(note => C5,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), (note => A4,  time_t => ov4_t1),
		(note => B4,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), (note => Cs5, time_t => ov4_t1), 
		(note => D5,  time_t => ov4_t1), (note => D5,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), 
		(note => D5,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1), (note => Cs5, time_t => ov4_t1),
		(note => Cs5, time_t => ov4_t1), (note => D5,  time_t => ov4_t1), (note => E5,  time_t => ov4_t1),
		(note => E5,  time_t => ov4_t1), (note => Fs5, time_t => ov4_t1), (note => Cs5, time_t => ov4_t1),
		(note => E5,  time_t => ov4_t1), (note => D5,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1),
		(note => Cs4, time_t => ov4_t1), (note => D5,  time_t => ov4_t1), (note => D5,  time_t => ov4_t1),
		(note => B4,  time_t => ov4_t1), (note => D5,  time_t => ov4_t1), (note => B4,  time_t => ov4_t1),
		(note => Cs5, time_t => ov4_t1), (note => Cs5, time_t => ov4_t1), (note => D5,  time_t => ov4_t1),
		(note => E5,  time_t => ov4_t1), (note => E5,  time_t => ov4_t1), (note => D5,  time_t => ov4_t1)
	);

	--Zelda
	constant music4 : vetor_zelda := (
		(note => PAUSA, time_t => ov1_t1),
...
begin
	...
	L2: process(current_state, duration, stop_in, play_in, selection, current_music)
 	...
 	begin
  		...
  		case selection is
    		when "00" => limite := 32; -- A-ha - Take On Me
    		when "01" => limite := 61; -- Pokemon - Cerulean Theme
    		when "10" => limite := 45; -- Frank Sinatra - My Way
    		when "11" => limite := 41; -- Zelda - Lost Woods
		end case;
